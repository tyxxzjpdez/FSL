...
class RelationNet(MetaTemplate):
    def __init__(self, model_func, n_way, n_support, loss_type='mse', use_cuda=True, adaptation=False):
        super(RelationNet, self).__init__(model_func, n_way, n_support, use_cuda=use_cuda, adaptation=adaptation)
        self.loss_type = loss_type  # 'softmax'# 'mse'
        self.relation_module = RelationModule(input_size=self.feat_dim, hidden_size=8, loss_type=self.loss_type)
    ...
    def set_forward(self, x, is_adaptation=False):
        if is_adaptation:
            z_support = x[:, :self.n_support]  # [N, S, d]
            z_query = x[:, self.n_support:]  # [N, Q, d]
        else:
            z_support, z_query = self.parse_feature(x, is_adaptation=is_adaptation)
        z_proto = z_support.reshape(self.n_way, self.n_support, *self.feat_dim).mean(1)  # [N,c,w,h]
        z_query = z_query.reshape(self.n_way * self.n_query, *self.feat_dim)  # [N*Q,c,w,h]
        z_proto_ext = z_proto.unsqueeze(0).repeat(self.n_query * self.n_way, 1, 1, 1, 1)  # [N*Q,N,c,w,h]
        z_query_ext = z_query.unsqueeze(1).repeat(1, self.n_way, 1, 1, 1)  # [N*Q,N,c,w,h]
        extend_final_feat_dim = self.feat_dim.copy()  # [c,w,h]
        extend_final_feat_dim[0] *= 2  # [2*c,w,h]
        relation_pairs = torch.cat((z_proto_ext, z_query_ext), 2).reshape(-1, *extend_final_feat_dim)  # [N*N*Q,2c,w,h]
        relations = self.relation_module(relation_pairs).view(-1, self.n_way)
        return relations

    def set_forward_adaptation(self, x):  # overwrite parent function
        ...
        for epoch in range(100):
            ...
            scores = self.set_forward(sub_x, is_adaptation=True)
            if self.loss_type == 'mse':
                y_oh = utils.one_hot(y, self.n_way).cuda()
                loss = self.loss_fn(scores, y_oh)
            else:
                loss = self.loss_fn(scores, y.cuda())
            loss.backward()
            set_optimizer.step()
    ...
    def set_forward_loss(self, x):
        y = torch.from_numpy(np.repeat(range(self.n_way), self.n_query))  # [0 0 0 1 1 1 2 2 2 3 3 3 4 4 4]
        scores = self.set_forward(x)
        if self.loss_type == 'mse':
            y_oh = utils.one_hot(y, self.n_way).cuda()
            return self.loss_fn(scores, y_oh)
        else:
            return self.loss_fn(scores, y.long().cuda())
...
class RelationConvBlock(nn.Module):
    def __init__(self, indim, outdim, padding=0):
        ...
        self.indim = indim
        self.outdim = outdim
        self.Conv = nn.Conv2d(indim, outdim, 3, padding=padding)
        self.BN = nn.BatchNorm2d(outdim, momentum=1, affine=True, track_running_stats=False)
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(2)
        self.parametrized_layers = [self.Conv, self.BN, self.relu, self.pool]
